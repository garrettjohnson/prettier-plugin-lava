...
import { toLavaHtmlAST, toLavaAST, LavaHtmlNode } from '~/parser/stage-2-ast';
import { deepGet } from '~/utils';

describe('Unit: Stage 2 (AST)', () => {
  let ast: any;
  describe('Unit: toLavaHtmlAST(text) and toLavaAST(text)', () => {
    [
      {
        title: 'toLavaHtmlAST(text)',
        fn: toLavaHtmlAST,
      },
      {
        title: 'toLavaAST(text)',
        fn: toLavaAST,
      },
    ].forEach((testContext) => {
      ...
      it(`${title} - should parse lava inline comments`, () => {
        ast = toAST(`{% #%}`);
        expectPath(ast, 'children.0').to.exist;
        expectPath(ast, 'children.0.type').to.eql('LavaTag');
      ...
      it(`${title} - should parse lava case as branches`, () => {
  ...
  describe('Unit: toLavaHtmlAST(text)', () => {
    let ast: any;
    ...
    it('should allow for at most 2 unclosed nodes in a LavaBranch', () => {
      // two empty nodes = ok
      let testCases = [
        '{% if cond %}<div>{% endif %}',
      ...
      for (const testCase of testCases) {
        expect(() => toLavaHtmlAST(testCase), testCase).not.to.throw();
      }
      ...
      for (const testCase of testCases) {
        expect(() => toLavaHtmlAST(testCase), testCase).to.throw(
          /Attempting to close LavaTag '[^']+' before HtmlElement 'c' was closed/,
        );
      }
      ...
      for (const testCase of testCases) {
        expect(() => toLavaHtmlAST(testCase), testCase).to.throw(
          /Attempting to close LavaTag '[^']+' before HtmlElement 'b' was closed/,
        );
      }
    });
  ...
  describe('Unit: toLavaAST(text)', () => {
    let ast: any;

    it('should parse nested unclosed tags', () => {
      ast = toLavaAST('{% for a in b %} <div> {% if true %}');
      ...
      expectPath(ast, 'children.0.children.0.children.1.type').to.eql('LavaTag');
    ...
    it('should parse unclosed conditions with assignments', () => {
      ...
      expectPath(ast, 'children.0.type').to.eql('LavaTag');
      expectPath(ast, 'children.0.name').to.eql('lava');

      expectPath(ast, 'children.0.markup.0.type').to.eql('LavaTag');
      expectPath(ast, 'children.0.markup.0.name').to.eql('assign');
      expectPath(ast, 'children.0.markup.0.markup.name').to.eql('var1');

      expectPath(ast, 'children.0.markup.1.type').to.eql('LavaTag');
  ...
  it('should allow for dangling html open tags inside branches when the conditions are right', () => {
    ['if', 'unless'].forEach((conditional) => {
      ast = toLavaHtmlAST(`
        ...
        {% end${conditional} %}
      `);
      expectPath(ast, 'children.0.children.0.type').to.equal('LavaBranch');
      ...
      expectPath(ast, 'children.0.children.1.type').to.equal('LavaBranch');
