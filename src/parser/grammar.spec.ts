...
describe('Unit: lavaHtmlGrammar', () => {
  ...
  describe(`Case: common to all grammars`, () => {
    it('should parse or not parse HTML+Lava', () => {
      grammars.forEach(({ grammar }) => {
        ...
        function expectMatchSucceeded(text: string) {
          const match = grammar.LavaHTML.match(text, 'Node');
    ...
    it('should parse or not parse {% lava %} lines', () => {
      grammars.forEach(({ grammar }) => {
        ...
        function expectMatchSucceeded(text: string) {
          const match = grammar.LavaStatement.match(text.trimStart(), 'Node');
          return expect(match.succeeded(), text);
        }
      });
    });
  ...
  describe('Case: placeholderGrammars', () => {
    ...
    function expectMatchSucceeded(text: string) {
      const match = placeholderGrammars.Lava.match(text.trimStart(), 'Node');
      return expect(match.succeeded(), text);
    }
  });
});
